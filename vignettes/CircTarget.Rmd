...
At first a simply filter of low count is applied to remove some noise.
...
To establish a possible impact of circRNA in gene expression using different molecular subtypes of human T-ALL we select a subset of circRNA defined as markers: only circRNA which expression can discretize two groups (DESeq adj. p-value<.1 and log fold-change>1.5) have been selected.
...
```{r, fig.cap = "t-SNE dimensionality reduction representation. K-means and hierarchical clustering are compared."}
...
gene_mark <- foreach::foreach(i=1:length(markers.circrnas), .combine = rbind) %dopar% {
  ...
  results.temp <- cbind(geneexpression(circ_idofinterest = markers.circrnas[i], circRNAs = circNormDeseq, 
                                       ...
                                       group = circtarget$group.df[, circ_id%in%markers.circrnas[i]],
