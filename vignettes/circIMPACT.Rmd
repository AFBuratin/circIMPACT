...
gene_mark_hipk3 <- as.data.table(gene_mark_hipk3)
gene_mark_tab = as.data.table(gene_mark)
gene_mark_hipk3 %>% dplyr::rename("Gene" = "gene_id", "logFC" = "log2FoldChange") %>% 
  arrange(padj) %>% 
...
# knitr::kable(gene_mark_hipk3 %>% dplyr::group_by(circRNA_markers, n.degs) %>% 
...
gene_mark_hipk3[gene_mark_hipk3$gene_id=="HPSE",]
head(gene_mark_hipk3)
...
registerDoParallel(cores=no_cores)
gene_class <- foreach::foreach(i=1:25) %dopar% {

  results.temp <- circIMPACT::gene_class(circ_idofinterest = top25[i], circRNAs = circNormDeseq,
