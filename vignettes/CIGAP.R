...
hues <- CIGAP::gg_color_hue(n.conditions)
...
CIGAP <- marker.selection(dat = circNormDeseq, dds = dds.filt.expr, sf = sf.filt, p.cutoff = 0.1, lfc.cutoff = 1, 
                                 method.d = "spearman", method.c = "complete", k = 2, median = TRUE)

## -----------------------------------------------------------------------------
circMark <- CIGAP$circ.targetIDS[5]
...
## ----message=FALSE------------------------------------------------------------
markers.circrnas <- CIGAP$circ.targetIDS
...
gene_mark <- foreach::foreach(i=1:5, .combine = rbind) %dopar% {
  ...
  results.temp <- data.frame(geneexpression(circ_idofinterest = markers.circrnas[i], circRNAs = circNormDeseq, 
                                       ...
                                       group = CIGAP$group.df[CIGAP$group.df$circ_id%in%markers.circrnas[i],],
                                       covariates = NULL), CIGAP = markers.circrnas[i])
}

gene_mark_hipk3 <- data.frame(geneexpression(circ_idofinterest = "11:33286412-33287511", circRNAs = circNormDeseq, 
                                       linearRNAs = filt.mat, colData = coldata.df, padj = 0.1, 
                                       group = CIGAP$group.df[CIGAP$group.df$circ_id%in%"11:33286412-33287511",],
                                       covariates = NULL), CIGAP = "11:33286412-33287511")
...
gene_mark %>% dplyr::rename("Gene" = "gene_id", "logFC" = "log2FoldChange") %>% 
  arrange(padj) %>% 
  select(CIGAP, Gene, logFC) %>% head(20) %>% 
  formattable::formattable(., align = c("c","c","c"), list(
          ...
          CIGAP = formattable::formatter("span", style = ~ formattable::style(color = "grey", font.weight = "bold")),
          logFC = CIGAP::color_tile3(digits = 3, n = 18, fun = "comma", palette = "PiYG")))
...
geneList <- gene_mark$log2FoldChange[gene_mark$CIGAP==markers.circrnas[5]]
geneList <- gene_mark_hipk3$log2FoldChange

# order gene list by foldchange
geneList = sort(geneList, decreasing = TRUE)
names(geneList) <- gene_mark$Gene[gene_mark$CIGAP==markers.circrnas[5]]
